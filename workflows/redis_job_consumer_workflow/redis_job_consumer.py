...
REDIS_COMPLETION_STATUS_KEY = "ARGO_STATUS:JOB_CONSUMER:" + CURRENT_YEARWEEK
...
def consume_jobs():
    try:
        ...
        for i in range(5):
            value_got = redis_connection.rpop(REDIS_JOB_QUEUE_KEY)
            print(
                "## Script-2: Got job from redis queue 'queue:argo_job_queue' -> ",
                value_got,
            )
            time.sleep(1)
    except Exception as e:
        print("Exception Occured while consuming jobs: %s" % (e))
